...
% modified by Daniel Buscombe Apr 2012 to make more efficient
% also to use qinterp2 (http://www.mathworks.com/matlabcentral/fileexchange/10772) 
...
if layers>1
  ...
  h = waitbar(0,'Applying Transformations....');
  for i=1:layers
    im_out(:,:,i) = ...
      qinterp2( orig.xi, orig.yi, double(im(:,:,i)), new.xi, new.yi );
    waitbar(i/layers,h)
  end
  close(h)
  
else
  im_out = qinterp2( orig.xi, orig.yi, double(im), new.xi, new.yi);  
end
